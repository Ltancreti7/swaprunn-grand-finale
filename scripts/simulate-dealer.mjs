...
(async () => {
  ...
  const badgeText = await page.evaluate(() => {
    const badge = document.querySelector('header .badge, header [class*="bg-white"]');
    if (badge) return badge.textContent?.trim();
    // Try other selectors used in AppHeader
    const el = document.querySelector('header .absolute .badge') || document.querySelector('header [class*="-right-1"]');
  ...
  fs.writeFileSync('/workspaces/swaprunn-grand-finale/sim-report.json', JSON.stringify(report, null, 2));
