...
export function AppHeader() {
  ...
  return <header className={cn("fixed top-0 left-0 right-0 z-[60] border-b transition-all", scrolled ? "bg-[#1A1A1A]/95 backdrop-blur-lg border-white/20 shadow-lg" : "bg-black/40 backdrop-blur-md border-white/10", !isVisible && "-translate-y-full", showBounce && "animate-header-bounce")} style={{
      ...
      <div className="flex items-center justify-between h-16 px-4 py-3">
        ...
        <div className="flex items-center justify-center min-w-[44px] h-[44px]">
          {shouldShowBack ? (
            <Button
              ...
              size="icon"
              onClick={handleBack}
              className="h-11 w-11 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white"
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <img src="/swaprunn-logo-2025.png?v=20251001" alt="SwapRunn" className={cn("w-auto transition-all duration-300", scrolled ? "h-20" : "h-28")} />
        </div>
