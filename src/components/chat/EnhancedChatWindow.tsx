...
import { Send, X, Paperclip, Image, Search, Check, CheckCheck, Phone, MessageSquare } from 'lucide-react';
...
export const EnhancedChatWindow = ({ 
  ...
  const loadMessages = async () => {
    try {
      ...
      const unread = messages.filter(msg => 
        msg.sender_type !== currentUserType && !msg.read_at
      ).length;
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      await sendMessage(
        file.type.startsWith('image/') ? 'Sent an image' : `Sent file: ${file.name}`,
  ...
  const sendMessage = async (message?: string, messageType: 'text' | 'image' | 'file' = 'text', fileUrl?: string, fileName?: string) => {
  ...
  const filteredMessages = messages.filter(msg =>
    !searchQuery || msg.message.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const getDeliveryStatus = (message: Message) => {
    if (message.sender_type === currentUserType) {
      return message.read_at ? <CheckCheck className="h-3 w-3 text-blue-500" /> : <Check className="h-3 w-3 text-gray-400" />;
    }
    return null;
  };
              ...
              <>
                {filteredMessages.map((message) => (
                  <div key={message.id} className="mb-4">
                    ...
                    />
                    <div className={`flex ${message.sender_type === currentUserType ? 'justify-end' : 'justify-start'} mt-1`}>
                ...
                {otherUserTyping && (
                  <div className="text-sm text-muted-foreground italic">
                    {currentUserType === 'driver' ? dealerName : driverName} is typing...
                  </div>
                )}
              </>
            )}
