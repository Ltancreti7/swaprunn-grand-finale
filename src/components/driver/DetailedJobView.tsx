...
import { Phone, MapPin, DollarSign, Clock, Car, Camera, X, ChevronDown, Gauge } from 'lucide-react';
...
export function DetailedJobView({ assignment, job, driverId, onJobUpdate }: DetailedJobViewProps) {
  ...
  const handleStartJob = async () => {
    ...
    if (preDeparturePhotos.length === 0) {
      toast({
        ...
        description: "Please upload at least one exterior photo before starting.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      setIsPunchedIn(true);
      setOpenSections({ ...openSections, preDeparture: false, tripDetails: true });
      toast({
        title: "Job Started",
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to start job. Please try again.",
        variant: "destructive"
      });
    }
  };
  ...
  const handleReturnToDealership = async () => {
    if (!endOdometer || !dealerPlate) {
      toast({
        title: "Missing Information",
        description: "Please fill in odometer and dealer plate before completing.",
        variant: "destructive"
      });
      return;
    }
  ...
  const handlePreDeparturePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
  ...
  return (
    <div className="space-y-3 px-4 py-2">
      ...
      <Collapsible 
        ...
        onOpenChange={(open) => setOpenSections({ ...openSections, vehicle: open })}
      ...
      >
        <CollapsibleTrigger className="w-full flex items-center justify-between p-5 hover:bg-white/5 transition-colors rounded-2xl group">
          ...
          <ChevronDown className={`h-5 w-5 text-gray-400 group-hover:text-[#E11900] transition-all ${openSections.vehicle ? 'rotate-180' : ''}`} />
        </CollapsibleTrigger>
      ...
      <Collapsible 
        ...
        onOpenChange={(open) => setOpenSections({ ...openSections, addresses: open })}
        className="bg-gradient-to-br from-neutral-800 to-neutral-900 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.3)]"
      >
        <CollapsibleTrigger className="w-full flex items-center justify-between p-5 hover:bg-white/5 transition-colors rounded-2xl group">
          <div className="flex items-center gap-3">
          ...
          <ChevronDown className={`h-5 w-5 text-gray-400 group-hover:text-[#E11900] transition-all ${openSections.addresses ? 'rotate-180' : ''}`} />
        </CollapsibleTrigger>
        <CollapsibleContent className="px-5 pb-5">
          <div className="space-y-4 mt-2">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-white/60 text-sm font-semibold">Pickup Address</p>
              </div>
              <p className="text-white/90 ml-4">{job.pickup_address || 'Not specified'}</p>
            </div>
            <div className="h-px bg-white/10"></div>
            <div>
              <div className="flex items-center gap-2 mb-2">
                <div className="w-2 h-2 rounded-full bg-[#E11900]"></div>
                <p className="text-white/60 text-sm font-semibold">Delivery Address</p>
              </div>
              <p className="text-white/90 ml-4">{job.delivery_address || 'Not specified'}</p>
            </div>
            {job.specific_time && (
              <>
                <div className="h-px bg-white/10"></div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/90 font-semibold">{job.specific_time}</p>
                  </div>
                </div>
              </>
            )}
      ...
      <Collapsible 
        ...
        onOpenChange={(open) => setOpenSections({ ...openSections, customer: open })}
        className="bg-gradient-to-br from-neutral-800 to-neutral-900 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.3)]"
      >
        <CollapsibleTrigger className="w-full flex items-center justify-between p-5 hover:bg-white/5 transition-colors rounded-2xl group">
          <div className="flex items-center gap-3">
          ...
          <ChevronDown className={`h-5 w-5 text-gray-400 group-hover:text-[#E11900] transition-all ${openSections.customer ? 'rotate-180' : ''}`} />
        </CollapsibleTrigger>
        <CollapsibleContent className="px-5 pb-5">
          <div className="space-y-3 mt-2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white/90 font-semibold">{job.customer_name || 'Not provided'}</p>
              </div>
            </div>
            <div className="h-px bg-white/10"></div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-white/80">{job.customer_phone || 'Not provided'}</span>
              </div>
              {job.customer_phone && (
                <Button
                  size="sm"
      ...
      {!isPunchedIn && (
        <Collapsible 
          ...
          onOpenChange={(open) => setOpenSections({ ...openSections, preDeparture: open })}
        ...
        >
          <CollapsibleTrigger className="w-full flex items-center justify-between p-5 hover:bg-white/5 transition-colors rounded-2xl group">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xl font-bold text-white">Pre-Departure Checklist</h3>
            </div>
            <ChevronDown className={`h-5 w-5 text-gray-400 group-hover:text-[#E11900] transition-all ${openSections.preDeparture ? 'rotate-180' : ''}`} />
          </CollapsibleTrigger>
          <CollapsibleContent className="px-5 pb-5">
            <div className="space-y-5 mt-2">
              <p className="text-white/70 text-sm">Complete these items before starting your delivery</p>
              
              {/* Starting Odometer */}
              <div className="space-y-2">
                <Label htmlFor="pre-start-odometer" className="text-white font-semibold text-base flex items-center gap-2">
              ...
              <div className="space-y-3">
                ...
                <p className="text-white/60 text-sm">Take photos of all sides of the vehicle</p>
                ...
                {preDeparturePhotos.length > 0 && (
                  <Badge className="bg-green-500/20 text-green-200 border-green-400/40 text-sm py-1 px-3">
                    ✓ {preDeparturePhotos.length} photo{preDeparturePhotos.length > 1 ? 's' : ''} uploaded
                  </Badge>
                )}
              </div>
            </div>
      ...
      {isPunchedIn && (
        <Collapsible 
          ...
          onOpenChange={(open) => setOpenSections({ ...openSections, tripDetails: open })}
          className="bg-gradient-to-br from-neutral-800 to-neutral-900 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.3)]"
        >
          <CollapsibleTrigger className="w-full flex items-center justify-between p-5 hover:bg-white/5 transition-colors rounded-2xl group">
            <div className="flex items-center gap-3">
            ...
            <ChevronDown className={`h-5 w-5 text-gray-400 group-hover:text-[#E11900] transition-all ${openSections.tripDetails ? 'rotate-180' : ''}`} />
          </CollapsibleTrigger>
          <CollapsibleContent className="px-5 pb-5">
            <div className="space-y-4 mt-2">
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="start-odometer" className="text-white/80 font-semibold">Start Odometer</Label>
                  <Input
                    id="start-odometer"
                    type="number"
                    value={startOdometer}
                ...
                <div className="space-y-2">
                  <Label htmlFor="end-odometer" className="text-white/80 font-semibold">End Odometer *</Label>
                  <Input
                    id="end-odometer"
                    type="number"
              ...
              <div className="space-y-2">
                <Label htmlFor="dealer-plate" className="text-white/80 font-semibold">Dealer Plate *</Label>
      ...
      {isPunchedIn && (
        <Collapsible 
          ...
          onOpenChange={(open) => setOpenSections({ ...openSections, proofOfDelivery: open })}
          className="bg-gradient-to-br from-neutral-800 to-neutral-900 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.3)]"
        >
          <CollapsibleTrigger className="w-full flex items-center justify-between p-5 hover:bg-white/5 transition-colors rounded-2xl group">
            <div className="flex items-center gap-3">
            ...
            <ChevronDown className={`h-5 w-5 text-gray-400 group-hover:text-[#E11900] transition-all ${openSections.proofOfDelivery ? 'rotate-180' : ''}`} />
          </CollapsibleTrigger>
          <CollapsibleContent className="px-5 pb-5">
            <div className="space-y-4 mt-2">
              <div className="space-y-2">
                <Label htmlFor="photo-upload" className="text-white/80 font-semibold">Upload Photo</Label>
                <Input
                  id="photo-upload"
                  type="file"
                  accept="image/*"
              ...
              <div className="space-y-2">
                <Label htmlFor="signature" className="text-white/80 font-semibold">Customer Signature</Label>
                <Input
                  id="signature"
                  type="text"
      ...
      <Collapsible 
        ...
        onOpenChange={(open) => setOpenSections({ ...openSections, payout: open })}
      ...
      >
        <CollapsibleTrigger className="w-full flex items-center justify-between p-5 hover:bg-white/5 transition-colors rounded-2xl group">
          ...
          <ChevronDown className={`h-5 w-5 text-gray-400 group-hover:text-[#E11900] transition-all ${openSections.payout ? 'rotate-180' : ''}`} />
        </CollapsibleTrigger>
        <CollapsibleContent className="px-5 pb-5">
            ...
            {isPunchedIn && (
              <div className="text-right">
                ...
                <p className="text-2xl font-semibold text-white">
                  {assignment.started_at ? Math.floor((Date.now() - new Date(assignment.started_at).getTime()) / 60000) : 0} min
                </p>
              </div>
            )}
          </div>
