...
import { Calendar, DollarSign, MapPin, Clock, TrendingUp, BarChart3 } from 'lucide-react';
...
export function JobHistoryDashboard({ driverId, jobs }: JobHistoryDashboardProps) {
  ...
  const calculateStats = () => {
    ...
    const filtered = jobs.filter(job => new Date(job.created_at) >= startDate);
    setFilteredJobs(filtered);

    const completed = filtered.filter(job => job.status === 'completed');
    const totalEarnings = completed.reduce((sum, job) => sum + (job.actual_pay_cents || job.estimated_pay_cents || 0), 0);
    const totalMiles = completed.reduce((sum, job) => sum + job.distance_miles, 0);
    const totalHours = completed.reduce((sum, job) => {
      if (job.started_at && job.ended_at) {
        const hours = (new Date(job.ended_at).getTime() - new Date(job.started_at).getTime()) / (1000 * 60 * 60);
        return sum + hours;
      }
      return sum;
    }, 0);
    ...
    const weeklyEarnings = Array.from({ length: 7 }, (_, i) => {
      ...
      return weekJobs.reduce((sum, job) => sum + (job.actual_pay_cents || job.estimated_pay_cents || 0), 0) / 100;
    }).reverse();

    const jobsByStatus = filtered.reduce((acc, job) => {
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span>{stats.totalHours > 0 ? formatCurrency(stats.totalEarnings / stats.totalHours) : '$0'}/hr</span>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <div key={job.id} className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <div className="text-right">
                  <p className="font-medium text-green-600">
                    {formatCurrency((job.actual_pay_cents || job.estimated_pay_cents || 0) / 100)}
                  </p>
