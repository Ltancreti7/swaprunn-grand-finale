...
import { ProfessionalJobService, JobStatusUpdateRequest } from '@/services/professionalJobService';
...
export const useProfessionalJob = ({ jobId, enableRealtime = false }: UseProfessionalJobOptions) => {
  ...
  const assignJob = useCallback(async (driverId?: string, preference: 'auto' | 'manual' = 'manual') => {
  ...
  // Start job (driver)
  const startJob = useCallback(async (assignmentId?: string, location?: { latitude: number; longitude: number }) => {
    return updateJobStatus('in_progress', { assignmentId, location });
  }, [updateJobStatus]);

  // Complete job (driver)
  const completeJob = useCallback(async (assignmentId?: string, location?: { latitude: number; longitude: number }) => {
    return updateJobStatus('completed', { assignmentId, location });
  }, [updateJobStatus]);

  // Cancel job (dealer or driver)
  const cancelJob = useCallback(async (assignmentId?: string, reason?: string) => {
  ...
  useEffect(() => {
    ...
    const cleanup = ProfessionalJobService.setupJobMonitoring(jobId, (update) => {
      setJobUpdates(prev => [...prev, {
        ...update,
        timestamp: new Date().toISOString()
      }]);

      // Show toast for important updates
      if (update.type === 'job_update' && update.data.status !== update.old.status) {
        toast({
          title: "Job Status Changed",
          description: `Status updated to ${update.data.status}`,
        });
      } else if (update.type === 'assignment_update' && update.event === 'INSERT') {
        toast({
