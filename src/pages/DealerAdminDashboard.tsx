...
import { Users, UserPlus, Truck, Calendar, Settings, BarChart3, Crown, Phone, Mail, Star, MapPin } from "lucide-react";
...
const DealerAdminDashboard = () => {
  ...
  return (
    <>
      ...
      <meta name="description" content="Admin dashboard for managing drivers, staff, and dealership operations on SwapRunn." />
      ...
      <div className="min-h-screen" style={{
        ...
        <div className="relative z-10 pt-16">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {/* Quick Actions */}
            <div className={`grid gap-4 mb-8 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}>
              <MobileActionButton
            ...
            <div className={`grid gap-6 mb-8 ${isMobile ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4'}`}>
              ...
              >
                <div className="text-2xl font-bold">{thisMonthJobs.length}</div>
              </MobileOptimizedCard>
            </div>
                ...
                <div className="space-y-3">
                  {jobs.slice(0, 5).map((job) => (
                    <MobileTouchFeedback key={job.id} variant="subtle">
                      <div className={`flex justify-between items-center bg-white/5 ${isMobile ? 'p-4 rounded-2xl' : 'p-3 rounded-lg'}`}>
                        <div>
              ...
              {drivers.map((driver) => (
                <MobileOptimizedCard key={driver.id}>
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          ...
                          <div className={`flex items-center gap-4 text-sm text-muted-foreground ${isMobile ? 'flex-col items-start gap-1' : ''}`}>
                            {driver.email && (
                              <div className="flex items-center gap-1">
                                <Mail className="w-3 h-3" />
                                <span className={isMobile ? 'text-xs' : ''}>{driver.email}</span>
                              </div>
                            )}
                            {driver.phone && (
                              <div className="flex items-center gap-1">
                                <Phone className="w-3 h-3" />
                                <span className={isMobile ? 'text-xs' : ''}>{driver.phone}</span>
                              </div>
                            )}
                          </div>
                        </div>
                      </div>
                      <div className={`flex items-center gap-4 text-sm ${isMobile ? 'flex-col items-start gap-2' : ''}`}>
                        <div className="flex items-center gap-1">
                          <Star className="w-3 h-3 text-yellow-400" />
                          <span>{driver.rating_avg ? driver.rating_avg.toFixed(1) : '5.0'}</span>
                          <span className="text-muted-foreground">({driver.rating_count || 0} reviews)</span>
                        </div>
                        <div className="flex items-center gap-1">
                    ...
                    <div className={`flex gap-2 ${isMobile ? 'flex-col' : 'flex-col'}`}>
                      <Badge 
