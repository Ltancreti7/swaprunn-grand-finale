...
import { Phone, Mail, Calendar, MapPin, Clock, Star, Bell, AlertTriangle, Search, X, RefreshCw, Check, Navigation, User, ChevronDown, DollarSign } from 'lucide-react';
...
function DriverPersonalProfile() {
  ...
  const fetchDriverData = async () => {
    ...
    try {
      ...
      const {
        data: driver,
        error: driverError
      } = await supabase.from('drivers').select('*').eq('id', userProfile.driver_id).single();
      ...
      const {
        ...
        error: assignmentsError
          ...
          *,
          jobs!inner(*)
        `).eq('driver_id', userProfile.driver_id).in('jobs.status', ['assigned', 'in_progress']).order('created_at', {
        ascending: false
      });
      ...
      const {
        ...
        error: completedError
          ...
          *,
          jobs!inner(*)
        `).eq('driver_id', userProfile.driver_id).eq('jobs.status', 'completed').order('created_at', {
        ascending: false
      }).limit(10);
      ...
      } = await supabase.rpc('get_open_jobs_for_drivers');
  ...
  const handleJobAccept = async (jobId: string) => {
    ...
    try {
      ...
      toast({
        title: "Job Accepted!",
        description: "You've successfully accepted this job. Check your upcoming drives."
      });
  ...
  if (loading) {
    return <div className="min-h-screen relative bg-black" style={{
      ...
      backgroundAttachment: 'scroll'
        ...
        <JobNotificationBanner show={showAlert} dealerName={latestJob?.dealer_name || "dealership"} onDismiss={clearAlert} />
    ...
    backgroundAttachment: 'fixed'
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6 pt-24 py-6 md:py-8">
        <div className="space-y-6 mx-0 px-px my-[14px]">
          {/* New Request Modal */}
          <NewRequestModal isOpen={showAlert} onDismiss={clearAlert} job={latestJob} />

          {/* Persistent Drive Notification */}
          {userProfile?.driver_id && <PersistentDriveNotification driverId={userProfile.driver_id} />}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full animate-fade-in">
              <div className="w-full mb-6 py-0 my-0 mt-4">
                <TabsList className="w-full grid grid-cols-4 gap-3 bg-transparent p-0 h-auto">
                  ...
                  <TabsTrigger value="new-requests" onClick={handleViewNewRequests} className="relative">
                    Requests
                    {newJobsCount > 0 && <Badge className="absolute -top-2 -right-2 bg-white text-[#E11900] font-bold px-2 py-1 text-xs rounded-full min-w-[20px] h-[20px] flex items-center justify-center shadow-md animate-pulse border border-[#E11900]">
                        {newJobsCount}
                      </Badge>}
                  </TabsTrigger>
                  <TabsTrigger value="upcoming" className="relative">
                    Up Next
                    {upcomingAssignments.length > 0 && <Badge className="absolute -top-2 -right-2 bg-white text-[#E11900] font-bold px-2 py-1 text-xs rounded-full min-w-[20px] h-[20px] flex items-center justify-center shadow-md border border-[#E11900]">
              ...
              <Card className="bg-white/10 backdrop-blur-sm border-white/20 shadow-lg rounded-2xl overflow-hidden">
                <CardContent className="p-0">
                  {/* Profile Tab */}
                  <TabsContent value="profile" className="p-6 space-y-6 animate-fade-in">
                    <div className="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-12">
                      ...
                      <div className="flex-1 w-full space-y-6">
                        <div className="space-y-5 text-center md:text-left">
                          ...
                          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 pt-3">
                            ...
                            <Badge className="bg-gradient-to-r from-blue-500 to-blue-600 text-white border-0 shadow-[0_4px_12px_rgba(59,130,246,0.3)] hover:shadow-[0_6px_16px_rgba(59,130,246,0.4)] hover:scale-105 transition-all duration-200 flex items-center gap-2 text-sm px-4 py-2 font-bold">
                              ...
                              {driverData?.rating_avg?.toFixed(1) || '5.0'} ({driverData?.rating_count || 0})
                            </Badge>
                            
                            {driverData?.day_off && <Badge className="bg-gradient-to-r from-orange-500 to-orange-600 text-white border-0 shadow-[0_4px_12px_rgba(249,115,22,0.3)] hover:shadow-[0_6px_16px_rgba(249,115,22,0.4)] hover:scale-105 transition-all duration-200 text-sm px-4 py-2 font-bold">
                                Day Off: {driverData.day_off.charAt(0).toUpperCase() + driverData.day_off.slice(1)}
                              </Badge>}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 pt-4">
                          <Button onClick={() => setIsEditProfileOpen(true)} className="w-full sm:w-auto h-12 bg-gradient-to-r from-gray-800 to-gray-900 hover:from-black hover:to-gray-800 text-white text-sm font-bold rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.4)] transition-all duration-300 hover:scale-105" variant="default">
                            Edit Profile
                          </Button>
                        </div>
                      </div>
                  ...
                  {/* New Requests Tab */}
                  <TabsContent value="new-requests" className="p-6 animate-fade-in">
                    {availableJobs.length === 0 ? <div className="text-center py-20">
                        <Clock className="h-16 w-16 text-white/40 mx-auto mb-4" />
                        <h3 className="text-2xl font-bold text-white mb-3">No Open Requests</h3>
                        <p className="text-white/70 text-lg mb-6">Check back soon for new delivery opportunities</p>
                        <Button onClick={handleRefreshJobs} className="bg-gradient-to-r from-gray-800 to-gray-900 hover:from-black hover:to-gray-800 text-white font-bold rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.4)] transition-all duration-300 hover:scale-105">
                            ...
                            <div className="bg-gradient-to-br from-gray-900/95 to-black border border-white/30 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300">
                                  ...
                                  <div className="space-y-2">
                                    <div className="flex items-center gap-2 text-sm text-white/80">
                                      ...
                                      <span className="text-white/70 truncate">
                                        {job.pickup_address.split(',').slice(1, 3).join(',').trim()}
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-2 text-sm text-white/80">
                                      ...
                                      <span className="text-white/70 truncate">
                                        {job.delivery_address.split(',').slice(1, 3).join(',').trim()}
                                      </span>
                                    </div>
                              ...
                              <CollapsibleContent>
                                <div className="border-t border-white/20 p-4 space-y-4 bg-black/20">
                                    ...
                                    <div className="bg-gray-800/80 border border-gray-700 rounded-xl p-3">
                                      <div className="flex items-start gap-2">
                                        ...
                                        <div className="flex-1">
                                          <p className="text-xs text-white/60 font-bold mb-1">PICKUP</p>
                                          <p className="text-sm text-white">{job.pickup_address}</p>
                                        </div>
                                      </div>
                                    </div>
                                    <div className="bg-gray-800/80 border border-gray-700 rounded-xl p-3">
                                      <div className="flex items-start gap-2">
                                        ...
                                        <div className="flex-1">
                                          <p className="text-xs text-white/60 font-bold mb-1">DELIVERY</p>
                                          <p className="text-sm text-white">{job.delivery_address}</p>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  {/* Details */}
                                  {(job.customer_name || job.customer_phone || job.notes) && (
                                    <div className="space-y-2">
                                      {job.customer_name && (
                                      ...
                                      {job.customer_phone && (
                                        <div className="flex items-center gap-2 text-sm">
                                          ...
                                          <span className="text-white">{job.customer_phone}</span>
                                        </div>
                                      )}
                                      {job.notes && (
                                        <div className="bg-gray-800/80 border border-gray-700 rounded-xl p-3">
                                          <p className="text-xs text-white/60 font-bold mb-1">NOTES</p>
                  ...
                  <TabsContent value="upcoming" className="p-6 animate-fade-in">
                    {upcomingAssignments.length === 0 ? <div className="text-center py-16 md:py-20">
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-8 max-w-md mx-auto">
                          ...
                          <h3 className="text-xl font-bold text-white mb-2 mx-0 px-[22px] my-[4px] py-[11px]">No Upcoming Jobs</h3>
                        ...
                        {upcomingAssignments
                          ...
                          .map(assignment => {
                            ...
                            const statusColor = statusColors[job.status as keyof typeof statusColors] || 'bg-gray-500 text-white';
                              ...
                              <CollapsibleTrigger className="w-full p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                                <div className="flex-1 text-left">
                                  <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <Badge className={`${statusColor} border-0 text-xs px-2 py-1`}>
                  ...
                  <TabsContent value="completed" className="p-6 animate-fade-in">
                    {completedJobs.length === 0 ? <div className="text-center py-16 md:py-20">
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-8 max-w-md mx-auto">
                          <Check className="h-16 w-16 text-white/30 mx-auto mb-4" />
                          <h3 className="text-xl font-bold text-white mb-2">No Completed Jobs</h3>
                          <p className="text-white/60">Your completed deliveries will appear here</p>
                        </div>
                      </div> : <div className="w-full overflow-x-auto pb-6">
                        <div className="flex gap-4 px-4">
                          {completedJobs.map(job => <Card key={job.id} className="flex-shrink-0 w-[320px] overflow-hidden bg-white/10 backdrop-blur-sm border-white/20 rounded-2xl hover:bg-white/15 transition-all duration-300 shadow-lg">
                              <CardContent className="p-5">
                                <div className="flex items-start justify-between mb-4">
                                  <div className="flex-1">
                                    <h3 className="font-bold text-base text-white mb-1 truncate">{formatJobTitle(job)}</h3>
                                    <p className="text-xs text-white/60 font-medium">{new Date(job.created_at).toLocaleDateString()}</p>
                                  </div>
                                ...
                                <div className="space-y-2.5 text-sm">
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <p className="text-white/60 text-xs font-semibold uppercase tracking-wide mb-0.5">Pickup</p>
                                      <p className="text-white font-medium text-xs truncate">{job.pickup_address}</p>
                                    </div>
                                  </div>
                                  <div className="flex items-start gap-2">
                                    <MapPin className="h-4 w-4 text-[#E11900] mt-0.5 flex-shrink-0" />
                                    <div className="flex-1 min-w-0">
                                      <p className="text-white/60 text-xs font-semibold uppercase tracking-wide mb-0.5">Delivery</p>
                                      <p className="text-white font-medium text-xs truncate">{job.delivery_address}</p>
                                    </div>
                                  </div>
                                  <div className="flex items-center justify-between pt-2 border-t border-white/10">
                                    <div className="flex items-center gap-2">
                                      <Clock className="h-4 w-4 text-blue-400 flex-shrink-0" />
                                      <span className="text-white font-semibold text-xs">{job.distance_miles} miles</span>
                                    </div>
                                  </div>
                                </div>
                              </CardContent>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-3">
              <img src="/swaprunn-logo-2025.png?v=20251001" alt="SwapRunn" className="h-7 w-auto" />
      ...
      {driverData && <EditDriverProfile isOpen={isEditProfileOpen} onClose={() => setIsEditProfileOpen(false)} driverData={driverData} onUpdate={handleProfileUpdate} />}
    </div>;
}
