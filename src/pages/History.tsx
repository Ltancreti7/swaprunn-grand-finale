...
const History = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen relative" style={{backgroundImage: `url(${mapBackgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
  ...
  return (
    <div className="min-h-screen relative" style={{backgroundImage: `url(${mapBackgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto pt-24 py-8 px-4 sm:px-6 lg:px-8">
        ...
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold text-text-primary">Job History</h1>
          <p className="text-text-secondary mt-1">View all completed delivery and swap jobs</p>
        </div>
          ...
          {completedJobs.length === 0 ? (
            <Card>
              <CardContent className="pt-6 text-center">
                ...
                <h3 className="text-lg font-medium text-text-primary mb-2">No Completed Jobs</h3>
                <p className="text-text-secondary">
                  Completed jobs will appear here once drivers finish their deliveries.
                </p>
              </CardContent>
            </Card>
          ) : (
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <CardTitle className="flex items-center space-x-3">
                        <span className="text-lg">{job.type} - {job.customer_name}</span>
                      ...
                      <div className="text-sm text-text-secondary">
                        {job.ended_at ? new Date(job.ended_at).toLocaleDateString() : 'N/A'}
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <span className="text-text-secondary ml-1">{job.driver_name || 'N/A'}</span>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                    ...
                    {job.notes && (
                      <div className="text-sm">
                        ...
                        <div className="text-text-secondary mt-1">{job.notes}</div>
                      </div>
                    )}
                    ...
                    <div className="text-xs text-text-secondary pt-2 border-t border-border">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        ...
                        {job.started_at && <div>Started: {new Date(job.started_at).toLocaleDateString()}</div>}
                        {job.ended_at && <div>Completed: {new Date(job.ended_at).toLocaleDateString()}</div>}
                      </div>
                    </div>
                  </CardContent>
                </Card>
