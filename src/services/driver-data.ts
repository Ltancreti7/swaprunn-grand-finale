...
export const getDriver = async (driverId: string): Promise<DriverProfile | null> => {
  if (USE_MOCK_DATA) {
    await delay(500);
...
export const getTimecard = async (driverId: string, period = 'week'): Promise<TimecardData | null> => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/timecards?driverId=${driverId}&period=${period}`);
    return response.ok ? await response.json() : null;
  } catch (error) {
...
export const getEarnings = async (driverId: string, period = 'week'): Promise<EarningsData | null> => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/earnings?driverId=${driverId}&period=${period}`);
    return response.ok ? await response.json() : null;
  } catch (error) {
...
export const getJobs = async (driverId: string, limit = 20): Promise<JobData[]> => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/jobs?driverId=${driverId}&limit=${limit}`);
...
export const clockInOut = async (driverId: string, clockIn: boolean): Promise<boolean> => {
  if (USE_MOCK_DATA) {
    await delay(800);
    // Update mock data
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/timecards`, {
      ...
      body: JSON.stringify({ driverId, action: clockIn ? 'clock_in' : 'clock_out' })
    });
    return response.ok;
  } catch (error) {
...
export const findDriverByIdOrPhone = async (searchTerm: string): Promise<DriverProfile | null> => {
  if (USE_MOCK_DATA) {
    await delay(400);
    // Simple search in mock data
    const driver = Object.values(mockDrivers).find(d => 
      d.id === searchTerm || d.phone === searchTerm
    );
    return driver || null;
  }
  
  try {
    const response = await fetch(`${API_BASE_URL}/drivers/search?q=${encodeURIComponent(searchTerm)}`);
    return response.ok ? await response.json() : null;
  } catch (error) {
