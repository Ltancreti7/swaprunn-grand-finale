...
class MockStore {
  ...
  private drivers: Driver[] = [
    { id: 'driver1', name: 'John Smith', phone: '(555) 123-4567', status: 'Available' },
    { id: 'driver2', name: 'Sarah Johnson', phone: '(555) 987-6543', status: 'Available' },
    { id: 'driver3', name: 'Mike Wilson', phone: '(555) 456-7890', status: 'Available' },
  ];

  // Job Management
  createJob(jobData: Omit<Job, 'id' | 'createdAt' | 'trackingToken' | 'status'>): Job {
    const job: Job = {
  ...
  updateJobStatus(jobId: string, status: Job['status'], updates?: Partial<Job>): Job | null {
  ...
  getCompletedJobs(): Job[] {
    return this.jobs.filter(job => job.status === 'Completed')
                   .sort((a, b) => (b.clockedOutAt?.getTime() || 0) - (a.clockedOutAt?.getTime() || 0));
  }
}
