...
export const repairUserProfile = async () => {
  try {
    ...
    if (!userType) {
      throw new Error('Account setup incomplete: No user type found. Please contact support or re-register with the correct account type.');
    }

    // Validate that the user_type is one of the expected values
    if (userType !== 'dealer' && userType !== 'driver') {
      throw new Error(`Invalid user type: ${userType}. Expected 'dealer' or 'driver'.`);
    }
