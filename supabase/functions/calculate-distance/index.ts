...
serve(async (req) => {
  ...
  const { pickup_address, delivery_address }: DistanceRequest = await req.json()
    ...
    const distanceInMiles = Math.round((distanceInMeters * 0.000621371) * 10) / 10 // Convert to miles and round to 1 decimal
    
    // Convert duration to human readable format
    const durationSeconds = parseInt(route.duration.replace('s', ''))
    const hours = Math.floor(durationSeconds / 3600)
    const minutes = Math.floor((durationSeconds % 3600) / 60)
    const durationText = hours > 0 ? `${hours} hour${hours > 1 ? 's' : ''} ${minutes} min${minutes !== 1 ? 's' : ''}` : `${minutes} min${minutes !== 1 ? 's' : ''}`
