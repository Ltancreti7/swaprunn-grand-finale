...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Received driver signup data:', { full_name, phone, email, license_number });

    // Validate required fields
    if (!full_name || !phone || !email || !license_number) {
      console.error('Missing required fields');
      return new Response(
        JSON.stringify({ error: 'Missing required fields: full_name, phone, email, license_number' }), 
        { 
          status: 400, 
