...
serve(async (req) => {
  ...
  try {
    ...
    const { jobId, driverId, dealerPreference = 'auto' }: AssignmentRequest = await req.json();
    ...
    if (existingAssignment) {
      return new Response(
        JSON.stringify({ error: 'Job already assigned', assignedTo: existingAssignment.driver_id }),
    ...
    if (!driverId && dealerPreference === 'auto') {
      ...
      for (const driver of drivers) {
        ...
        const completedJobs = assignments?.filter((a: any) => a.jobs?.status === 'completed').length || 0;
        ...
        if (withinRange) {
          ...
          const totalScore = (ratingScore * 0.4) + (completionScore * 0.4) + (experienceScore * 0.2);
      ...
      if (driverScores.length === 0) {
        return new Response(
          JSON.stringify({ error: 'No qualified drivers available for this job' }),
          { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (!assignedDriverId) {
      return new Response(
        JSON.stringify({ error: 'No driver specified and auto-assignment failed' }),
