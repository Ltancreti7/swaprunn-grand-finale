...
serve(async (req) => {
  ...
  try {
    ...
    const { jobId, assignmentId, newStatus, userId, location, notes }: StatusUpdateRequest = await req.json();
    ...
    const canUpdate = 
      (userProfile?.user_type === 'dealer' && userProfile.dealer_id === job.dealer_id) ||
      (userProfile?.user_type === 'driver' && userProfile.driver_id === assignment.driver_id);
    ...
    switch (newStatus) {
      ...
      case 'completed':
        if (job.status !== 'in_progress') {
          return new Response(
            JSON.stringify({ error: 'Job must be in progress before completing' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
    ...
    try {
      await supabase.functions.invoke('send-job-notification', {
        body: {
          driverId: userProfile?.user_type === 'dealer' ? assignment.driver_id : undefined,
          dealerId: userProfile?.user_type === 'driver' ? job.dealer_id : undefined,
          jobId,
