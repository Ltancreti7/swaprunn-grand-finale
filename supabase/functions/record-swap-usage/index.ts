...
serve(async (req) => {
  ...
  try {
    ...
    const dealerUsage = new Map<string, { subscriptionId: string, count: number, records: any[] }>();
    ...
    for (const [dealerId, usage] of dealerUsage) {
      if (!stripeSecretKey || !usage.subscriptionId) {
        console.log(`Skipping Stripe reporting for dealer ${dealerId} (test mode or no subscription)`);
      ...
      if (!meteredItem) {
        console.error(`No metered item found for subscription ${usage.subscriptionId}`);
        continue;
      }
