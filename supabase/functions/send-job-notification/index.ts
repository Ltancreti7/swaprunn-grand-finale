...
serve(async (req) => {
  ...
  try {
    ...
    const { driverId, dealerId, jobId, notificationType, customMessage }: NotificationRequest = await req.json();
    ...
    switch (notificationType) {
      ...
      case 'status_update':
        ...
        message = customMessage || `Your job status has been updated to: ${job.status}`;
        break;
    ...
    if (driverId) {
      ...
      if (driver) {
        ...
        if (driver.email && preferences.email_notifications && preferences.job_updates) {
          console.log('Would send email to:', driver.email, 'Subject:', subject);
          notifications.push({ type: 'email', status: 'skipped', reason: 'Resend not configured' });
        }

        // SMS notification
        if (driver.phone && preferences.sms_notifications && preferences.job_updates) {
          try {
            ...
            notifications.push({ type: 'sms', status: 'sent', result: smsResult });
          } catch (smsError) {
            console.error('SMS notification error:', smsError);
            notifications.push({ type: 'sms', status: 'failed', error: smsError instanceof Error ? smsError.message : 'SMS failed' });
          }
        }
      }
    }
    ...
    if (dealerId) {
      ...
      if (dealer?.email) {
        ...
        console.log('Would send email to dealer:', dealer.email, 'Subject:', dealerSubject);
        notifications.push({ type: 'dealer_email', status: 'skipped', reason: 'Resend not configured' });
      }
    }
