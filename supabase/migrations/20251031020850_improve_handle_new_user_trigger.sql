/*
  ...
  1. Improvements
    ...
    - Transaction safety to ensure atomic operations
    - Validation checks before inserting records
    - Better null handling for metadata fields
    - Explicit error messages for debugging
    
  2. Security
    - SECURITY DEFINER with explicit search_path
    - Only runs for authenticated users
    - Validates user_type before creating records
    
  3. Logging
    - Logs all attempts and failures to help diagnose issues
...
BEGIN
  ...
  BEGIN
    ...
    RAISE LOG 'handle_new_user: Extracted metadata - name: %, company: %, phone: %', 
      user_full_name, user_company_name, user_phone;
  ...
  ELSIF user_type_value = 'driver' THEN
    BEGIN
      RAISE LOG 'handle_new_user: Creating driver record for user %', NEW.id;
      
      -- Create driver record with pending status for approval workflow
      ...
      RAISE LOG 'handle_new_user: Created driver record % for user %', driver_record_id, NEW.id;
...
COMMENT ON FUNCTION public.handle_new_user() IS 'Automatically creates dealer, driver, or swap_coordinator records when users sign up. Includes comprehensive error handling and logging.';
